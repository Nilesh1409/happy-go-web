...
class ApiService {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401 || response.status === 403) {
          ...
          throw new Error(
            `Auth error ${response.status}: ${text.slice(0, 200)}`,
          );
        }
  ...
  async uploadDLImage(file) {
    ...
    const url = `${this.baseURL}/api/users/dl-image`;
    const token =
      typeof window !== "undefined" ? localStorage.getItem("token") : null;

    // Use fetch directly — do NOT set Content-Type so the browser can
  ...
  async validateReferralCode(referralCode) {
    return this.request("/api/referrals/validate", {
      method: "POST",
  ...
  // Payment APIs
  async createBookingPayment(bookingId, paymentType = "full", useWallet = false) {
    return this.request(`/api/payments/booking/${bookingId}`, {
      method: "POST",
      body: JSON.stringify({ paymentType, useWallet }),
    });
  }
  ...
  async initiateAadhaarVerification(payload) {
    return this.request("/api/verification/aadhaar/initiate", {
      method: "POST",
      body: JSON.stringify(payload),
    });
  }

  async checkAadhaarStatus() {
    return this.request("/api/verification/aadhaar/status", {
      method: "GET",
    });
  }

  async completeAadhaarVerification() {
    return this.request("/api/verification/aadhaar/complete", {
      method: "POST",
      body: JSON.stringify({}),
    });
  }
  ...
  async getHostelDetails(hostelId, params = {}) {
    const queryString = new URLSearchParams(params).toString();
    return this.request(
      `/api/hostels/${hostelId}${queryString ? `?${queryString}` : ""}`,
    );
  }
